...
/* ===== STATE ===== */
const KUNCI_DB='taniku_db_v1',KUNCI_SESI='taniku_sesi';
...
function rute(){
  ...
  // Kalau di area admin/pembeli tapi belum login
  if (b[0] === 'admin' || b[0] === 'pembeli') {
    if (!SESI) {
      location.hash = '#/masuk';
      return;
    }
    if (b[0] === 'admin' && SESI.peran !== 'admin') {
      location.hash = '#/pembeli';
      return;
    }
    if (b[0] === 'pembeli' && SESI.peran !== 'pembeli') {
      location.hash = '#/admin';
      return;
    }
  }
  // Kalau sudah login tapi di halaman masuk/daftar
  else if (b[0] === 'masuk' || b[0] === 'daftar' || b[0] === '') {
    if (SESI) {
      location.hash = SESI.peran === 'admin' ? '#/admin' : '#/pembeli';
      return;
    }
  }
  bounceKeranjang();
...
function render(){
  ...
  if (!b.length || b[0] === 'masuk') {
    ...
    app.innerHTML = layout(SESI, MENU_ADMIN, halamanAdmin(b.slice(1)));
  } else if (b[0] === 'pembeli') {
    app.innerHTML = layout(SESI, MENU_PEMBELI, halamanPembeli(b.slice(1)));
  } else {
    app.innerHTML = halamanMasuk();
  }
  ...
  // Panggil cuaca real setelah render halaman cuaca
...
/* ===== ADMIN ===== */
function halamanAdmin(sub){const r=sub[0]||'dasbor';
  if(r==='produk')return adminProduk();if(r==='persediaan')return adminPersediaan();
  if(r==='pesanan')return adminPesanan();if(r==='mitra')return adminMitra();
  if(r==='promo')return adminPromo();
  if(r==='cuaca')return adminCuaca();if(r==='notifikasi')return adminNotif();
  return adminDasbor()}
...
function adminNotif(){
  const list = DB.notifikasi.filter(n => n.peran === 'admin')
...
function notifItem(n){
  const ik = { pesanan: 'shopping-bag', produk: 'package', komplain: 'message-square', cuaca: 'cloud-sun', persediaan: 'boxes', mitra: 'users' };
  const wr = { pesanan: 'bg-blue', produk: 'bg-green', komplain: 'bg-yellow', cuaca: 'bg-blue', persediaan: 'bg-red', mitra: 'bg-green' };
...
/* ===== PEMBELI ===== */
function halamanPembeli(sub){const r=sub[0]||'dasbor';
  if(r==='katalog')return pembeliKatalog();if(r==='produk'&&sub[1])return pembeliDetail(sub[1]);
  if(r==='keranjang')return pembeliKeranjang();if(r==='checkout')return pembeliCheckout();
  if(r==='pesanan')return pembeliPesanan(sub);if(r==='cuaca')return pembeliCuaca();
  if(r==='notifikasi')return pembeliNotif();return pembeliDasbor()}
...
function pembeliNotif(){const list=DB.notifikasi.filter(n=>n.peran==='pembeli').sort((a,b)=>new Date(b.waktu)-new Date(a.waktu));
  return '<div class="page-head"><div><h1 class="page-title">Notifikasi</h1><p class="page-sub">'+list.filter(n=>!n.dibaca).length+' belum dibaca</p></div>'+
    '<div class="flex gap-2"><button class="btn btn-outline" data-aksi="tandai-semua"><i data-lucide="check-check"></i> Tandai Semua</button>'+
    '<button class="btn btn-danger" data-aksi="hapus-lama"><i data-lucide="trash-2"></i> Hapus Lama</button></div></div>'+
    '<div class="card">'+(list.length?list.map(n=>notifItem(n)).join(''):'<div class="empty"><div class="empty-icon"><i data-lucide="bell-off"></i></div><h3>Tidak Ada Notifikasi</h3></div>')+'</div>'}
...
function aksiBuatPesanan(){
  const subtotal=Number(document.querySelector('[data-aksi="buat-pesanan"]').dataset.subtotal)||0;
  ...
  const k=DB.keranjang[SESI.id]||{};
  const items=Object.keys(k).map(id=>{const p=DB.produk.find(x=>x.id===id);
    return p?{produkId:p.id,nama:p.nama,jumlah:k[id],harga:p.harga,satuan:p.satuan,emoji:p.emoji}:null}).filter(Boolean);
  if(!items.length){toast('Keranjang kosong.','error');return}
  for(const it of items){const p=DB.produk.find(x=>x.id===it.produkId);
    if(p.stok<it.jumlah){toast('Stok '+p.nama+' tidak cukup.','error');return}}

  // Kurangi stok sekarang (karena pesanan sudah fix)
  items.forEach(it=>{const p=DB.produk.find(x=>x.id===it.produkId);p.stok-=it.jumlah;p.terjual+=it.jumlah});
  ...
  const pesanan={
    id:uid('o'),nomor,pembeliId:SESI.id,pembeliNama:SESI.nama,item:items,
    subtotal,ongkir,diskon,biayaLayanan:bl,total,
    promoKode:promo?promo.kode:null,
    metodePembayaran:labelBayar,
    metodeBayarKode:metodeBayar,
    // Kalau COD → langsung diproses, kalau lain → menunggu bayar
    ...
    kurir:null,resi:null,
    estimasiTiba:'Besok, 12:00 - 15:00 WIB',
    tanggal:new Date().toISOString(),
    garisWaktu:[
      {tahap:'Pesanan Dibuat',waktu:tglWaktu(new Date()),selesai:true,catatan:'Sistem Taniku'},
      {tahap:metodeBayar==='cod'?'Menunggu Pengiriman':'Menunggu Pembayaran',
        waktu:tglWaktu(new Date()),selesai:true,
        catatan:metodeBayar==='cod'?'COD':'Silakan selesaikan pembayaran'},
      {tahap:'Pembayaran Diterima',waktu:'—',selesai:false,catatan:''},
      {tahap:'Menunggu Konfirmasi Gudang',waktu:'—',selesai:false,catatan:''}
    ]
  };
  ...
  DB.notifikasi.unshift({id:uid('n'),peran:'pembeli',tipe:'pesanan',dibaca:false,
    ...
    pesan:'Pesanan '+nomor+' sebesar '+rp(total)+' menunggu pembayaran via '+labelBayar+'.',
    waktu:new Date().toISOString(),tautan:'#/pembeli/pesanan/'+pesanan.id});
  DB.notifikasi.unshift({id:uid('n'),peran:'admin',tipe:'pesanan',dibaca:false,
    judul:'Pesanan Baru Masuk',
    pesan:nomor+' dari '+SESI.nama+' — '+rp(total)+' ('+(metodeBayar==='cod'?'COD':'Belum Bayar')+').',
    waktu:new Date().toISOString(),tautan:'#/admin/pesanan'});
  simpanDB();
  window.__PROMO_AKTIF=null;

  if(metodeBayar==='cod'){
    toast('Pesanan berhasil dibuat. Bayar saat terima barang.','success');
  } else {
    toast('Pesanan dibuat. Silakan selesaikan pembayaran.','success');
  }
  location.hash='#/pembeli/pesanan/'+pesanan.id;
}
...
function aksiKeluar(){
  SESI = null;
  simpanSesi(null);
  Api.setToken('');  // hapus token JWT
  location.hash = '#/masuk';
  toast('Anda telah keluar.', 'info');
}

/* Fungsi 2: Selesaikan pembayaran pesanan */
function aksiBayarPesanan(pesananId,metodeBayar){
  const o=DB.pesanan.find(x=>x.id===pesananId);if(!o)return;
  const nomor='TNK-'+o.id.slice(-6).toUpperCase();
...
/* Fungsi 3: Konfirmasi bayar → update status */
function aksiKonfirmasiBayar(){
  const b=window.__BAYAR;if(!b){toast('Data pembayaran hilang.','error');return}
  const o=DB.pesanan.find(x=>x.id===b.pesananId);if(!o){toast('Pesanan tidak ditemukan.','error');return}
  o.status='dibayar';
  ...
  if(!o.garisWaktu.some(g=>g.tahap==='Pembayaran Diterima')){
    o.garisWaktu.push({
      tahap:'Pembayaran Diterima',waktu:tglWaktu(new Date()),
      selesai:true,catatan:o.metodePembayaran
    });
  }
  DB.notifikasi.unshift({id:uid('n'),peran:'admin',tipe:'pesanan',dibaca:false,
...
/* ==== AKSI NOTIFIKASI ==== */
function aksiBacaNotif(id){
  const n = DB.notifikasi.find(x => x.id === id);
  if (!n) return;
  n.dibaca = true;
  simpanDB();
  if (n.tautan && n.tautan !== HALAMAN_KINI) {
...
function pasangEvent(){
  ...
  document.addEventListener('click',e=>{
    ...
    switch(a){
      case 'buka-menu':MENU_BUKA=true;render();break;
      case 'tutup-menu':MENU_BUKA=false;render();break;
      ...
      case 'filter-kategori':FILTER_KATEGORI=el.dataset.kategori;render();break;
      ...
      case 'tandai-semua':
        ...
        simpanDB();toast('Semua ditandai dibaca.','success');render();break;
      case 'hapus-lama':
        DB.notifikasi=DB.notifikasi.filter(n=>n.peran!==SESI.peran||!n.dibaca);
        simpanDB();toast('Notifikasi lama dihapus.','success');render();break;
      case 'baca-notif':aksiBacaNotif(id);break;
...
setTimeout(() => {
  ...
  toast('Produk Berhasil Disimpan', 'success');
    render();
      ...
      case 'buat-pesanan':aksiBuatPesanan();break;
      case 'bayar-pesanan':{
